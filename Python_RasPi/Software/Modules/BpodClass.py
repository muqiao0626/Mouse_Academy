...
class BpodObject(object):
    ...
    def updateCurrentDataFolder(self):
        df = os.path.join(self.dataFolder, self.subject, self.protocol, 'Session Data')
    ...
    def set_protocol(self, protStr):
        ...
        self.updateCurrentDataFolder()
        self.updateCurrentDataFile()
        protocolFolder = os.path.join(AcademyUtils.getSoftwareDir(), protStr)
        sys.path.append(protocolFolder)
        protocolMod = importlib.import_module(protStr)
        print('Protocol Module:', protocolMod)
        try:
    ...
    def runStateMachine(self):
        ...
        RawEvents.TrialSentTimestamp =  self.sentDateTime.timestamp() # Start-time of the trial in milliseconds
